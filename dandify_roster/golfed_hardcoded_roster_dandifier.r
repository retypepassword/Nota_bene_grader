...
write.csv(Reduce(function(d,i)`if`(!length(g("Seq",i)),d,rbind(d,data.frame(CRN=x(e[i]),Section=x(a(r,p("(//th[b='SEC'])[",i,"]",b,"*[1]/td[6]",sep=""))),sapply(y,g,i,simplify=F)))),seq(e),matrix(NA,0,14)),"roster.csv",row.names=F)
